...
public final class FrequencyLegendView: NSView {
    ...
    static let yellow   = NSColor(srgbRed: 1.0, green: 0.5, blue: 0.0, alpha: alpha)
    ...
    static let xxx      = NSColor(srgbRed: 0.5, green: 1.0, blue: 0.0, alpha: alpha)
    ...
    private var _bandwidthParam               : BandwidthParamTuple {  // given Bandwidth, return a Spacing & a Format
        return kBandwidthParams.filter { $0.high > _bandwidth && $0.low <= _bandwidth }.first ?? kBandwidthParams[0] }
    ...
    private let _segmentColors                = [green, yellow, blue, red, gray, brown, purple, xxx]
    ...
    func updateBandwidth(dragable: PanadapterViewController.Dragable) {
        ...
        let delta = ((dragable.previous.x - dragable.current.x) / frame.width)
        ...
        let newStartPercent = (dragable.frequency - newStart) / newBandwidth
        let freqError = (newStartPercent - dragable.percent) * newBandwidth
    ...
    func updateTnf(dragable: PanadapterViewController.Dragable) {
        ...
        if let tnf = dragable.object as? Tnf {
            
            // YES, drag or resize?
            if abs(deltaX) > abs(deltaY) {
                
                // drag
                tnf.frequency = Hz(dragable.current.x * _hzPerUnit) + _start
            } else {
                
                // resize
                //        tnf.width = UInt( max( Int(tnf.width) + Int(deltaY * CGFloat(_bandwidth) * kMultiplier), Int(Tnf.kWidthMin)) )
                tnf.width += Hz(deltaY * CGFloat(_bandwidth) * kMultiplier)
            }
        }
        // redraw the tnfs
        redraw()
    ...
    private func drawLegend(_ dirtyRect: NSRect) {
        ...
        for i in 0...numberOfMarks {
            ...
            if xPosition > 0 && xPosition + legendWidth < frame.width && xPosition - previousLegendPosition > 1.2 * legendWidth {
                // draw the legend
                legendLabel.draw(at: CGPoint( x: xPosition - (legendWidth/2), y: 1), withAttributes: _attributes)
