...
public final class DbLegendView: NSView {
    ...
    func updateDbmLevel(dragable: PanafallViewController.Dragable) {
        ...
        if dragable.original.y > frame.height/2 {
            ...
            _panadapter!.maxDbm += (dragable.previous.y - dragable.current.y)
            
        } else {
            
            // NO, update the min value
            _panadapter!.minDbm += (dragable.previous.y - dragable.current.y)
        }
    ...
    ///
    func updateLegendSpacing(gestureRecognizer: NSClickGestureRecognizer, in view: NSView) {
        var item: NSMenuItem!
    ...
    private func drawLegend(_ dirtyRect: NSRect) {
        ...
        for i in 0...numberOfLegends {
            ...
            lineLabel.draw(at: CGPoint(x: frame.width - width, y: yPosition), withAttributes: _attributes)
        }
        _path.strokeRemove()
