...
final class XritViewController: NSViewController {
            ...
            if $0.modifierFlags.contains(NSEvent.ModifierFlags.option) {
                ...
                DispatchQueue.main.async { [unowned self] in
                    if self._splitStepMode {
                        ...
                        self._stepTextField.integerValue = self._slice?.step ?? 10
                        self._stepStepper.integerValue = self._slice?.step ?? 10
                    }
                }
            }
            return $0
    ...
    private func addObservations() {
        if let slice = _slice {
            _observations = [
                slice.observe(\.ritEnabled, options: [.initial, .new]) { [weak self] (slice, change) in
                    self?.changeHandler(slice, change) },
                slice.observe(\.xitEnabled, options: [.initial, .new]) { [weak self] (slice, change) in
                    self?.changeHandler(slice, change) },
                slice.observe(\.ritOffset, options: [.initial, .new]) { [weak self] (slice, change) in
                    self?.changeHandler(slice, change) },
                slice.observe(\.xitOffset, options: [.initial, .new]) { [weak self] (slice, change) in
                    self?.changeHandler(slice, change)},
                slice.observe(\Slice.step, options: [.initial, .new]) { [weak self] (slice, change) in
