...
final class FlagViewController: NSViewController, NSTextFieldDelegate, NSGestureRecognizerDelegate {
    ...
    class func createFlag(for slice: xLib6000.Slice, and pan: Panadapter, on viewController: NSViewController) -> FlagViewController {
        ...
        let storyboard = NSStoryboard(name: "Flag", bundle: nil)
        
        // create a Flag View Controller & pass it needed parameters
        if let flagVc = storyboard.instantiateController(withIdentifier: "Flag") as? FlagViewController,
           let controlsVc = storyboard.instantiateController(withIdentifier: "Controls") as? ControlsViewController {
            
            // pass the FlagVc needed parameters
            flagVc.configure(panadapter: pan, slice: slice, controlsVc: controlsVc, flagVc: viewController)
...
//        parent.addSubview(flagVc.view, positioned: .above, relativeTo: parent)
    ...
    ///
    func configure(panadapter: Panadapter, slice: xLib6000.Slice, controlsVc: ControlsViewController, flagVc: NSViewController) {
        
        // save the params
        _panadapter = panadapter
        self.slice = slice
        self.controlsVc = controlsVc
        _viewController = flagVc
    ...
    @IBAction func controlsButtons(_ sender: NSButton) {
        ...
        if sender.boolState {
            ...
            if let sideViewController = _viewController as? SideViewController { sideViewController.setRxHeight(2 * FlagViewController.kLargeFlagHeight) }
    ...
    @IBAction func sliceLetterButton(_ sender: Any) {
        ...
        guard _viewController is PanadapterViewController else { return }
        ...
        if let panadapterViewController = _viewController as? PanadapterViewController { panadapterViewController.positionFlags() }
    }
...
extension FlagViewController {
    ...
    private func addObservations(slice: xLib6000.Slice, panadapter: Panadapter ) {
        ...
        _observations = [
            ...
            slice.observe(\.active, options: [.initial, .new, .old]) { [weak self] (slice, change) in
                if let old = change.oldValue, let new = change.newValue {
                    if old == false && new == true {
                        ...
                        NCtr.post(.sliceBecameActive, object: slice)
                    }
                }},
            
            slice.observe(\.mode, options: [.initial, .new]) { [weak self] (slice, _) in
            ...
            slice.observe(\.txEnabled, options: [.initial, .new]) { [weak self] (slice, _) in
                DispatchQueue.main.async {
            ...
            slice.observe(\.filterHigh, options: [.initial, .new]) { [weak self] (slice, change) in
                if let panVc = self?._viewController as? PanadapterViewController { panVc.redrawFrequencyLegend() }
            ...
            slice.observe(\.frequency, options: [.initial, .new]) { [weak self] (slice, _) in
                if let panVc = self?._viewController as? PanadapterViewController { panVc.redrawFrequencyLegend() }
                DispatchQueue.main.async { self?._frequencyField?.doubleValue = slice.frequency.intHzToDoubleMhz }},
            
            panadapter.observe(\.center, options: [.initial, .new]) { [weak self] (_, _) in
                if let panVc = self?._viewController as? PanadapterViewController {
                    // this is a Slice Flag, move the Flag(s)
                    panVc.positionFlags() }},
            
            panadapter.observe(\.bandwidth, options: [.initial, .new]) { [weak self] (_, _) in
                if let panVc = self?._viewController as? PanadapterViewController {
                    // this is a Slice Flag, move the Flag(s)
                    panVc.positionFlags() }},
    ...
    private func positionChange(_ object: Any, _ change: Any) {
        ...
        if let panVc = _viewController as? PanadapterViewController {
            DispatchQueue.main.async {
                // this is a Slice Flag, move the Flag(s)
                panVc.positionFlags()
            }
    ...
    @objc private func sliceMeterUpdated(_ note: Notification) {
        ...
        if let meter = note.object as? Meter, meter.group.objectId == slice?.id, meter.name == Meter.ShortName.signalPassband.rawValue {
            ...
            DispatchQueue.main.async { [self] in
                ...
                _sMeter.level = value
                _sMeterText.stringValue = sLevel
            }
        }
    }
}
