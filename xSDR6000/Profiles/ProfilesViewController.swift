...
//  }
  ...
  func controlTextDidChange(_ obj: Notification) {
    ...
    _loadButton.isEnabled = (_nameTextField.stringValue == "")
    _deleteButton.isEnabled = (_nameTextField.stringValue == "")
    _resetButton.isEnabled = (_nameTextField.stringValue == "")
  }
  ...
  private func reloadTable(select row: Int = -1) {
    ...
    // redraw
    var row = -1
    for i in 0..<_profiles[_profileType]!.list.count {
      if _profiles[_profileType]!.list[i] == _profiles[_profileType]!.selection {
        row = i
        break
      }
    }
