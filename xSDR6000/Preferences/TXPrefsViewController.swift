...
final class TXPrefsViewController: NSViewController {
    ...
    private func addObservations() {
        ...
        _observations = [
            // ----- Profile Strings -----
            txProfile!.observe(\.selection, options: [.initial, .new]) { [weak self] (profile, _) in
                self?.updateProfileStringValues(profile, \.selection) },
            
            // ----- Interlock Bools -----
            _interlock!.observe(\.rcaTxReqEnabled, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockBoolValues(interlock, \.rcaTxReqEnabled)},
            _interlock!.observe(\.accTxReqEnabled, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockBoolValues(interlock, \.accTxReqEnabled)},
            _interlock!.observe(\.accTxEnabled, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockBoolValues(interlock, \.accTxEnabled) },
            _interlock!.observe(\.tx1Enabled, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockBoolValues(interlock, \.tx1Enabled )},
            _interlock!.observe(\.tx2Enabled, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockBoolValues(interlock, \.tx2Enabled )},
            _interlock!.observe(\.tx3Enabled, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockBoolValues(interlock, \.tx3Enabled )},
            
            // ----- Interlock Ints -----
            _interlock!.observe(\.accTxDelay, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockIntValues(interlock, \.accTxDelay)},
            _interlock!.observe(\.timeout, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockIntValues(interlock, \.timeout)},
            _interlock!.observe(\.tx1Delay, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockIntValues(interlock, \.tx1Delay)},
            _interlock!.observe(\.tx2Delay, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockIntValues(interlock, \.tx2Delay)},
            _interlock!.observe(\.tx3Delay, options: [.initial, .new]) { [weak self] (interlock, _) in
                self?.updateInterlockIntValues(interlock, \.tx3Delay)},
            
            // ----- Transmit Bools -----
            _transmit!.observe(\.inhibit, options: [.initial, .new]) { [weak self] (transmit, _) in
                self?.updateTransmitBoolValues(transmit, \.inhibit )},
            _transmit!.observe(\.hwAlcEnabled, options: [.initial, .new]) { [weak self] (transmit, _) in
                self?.updateTransmitBoolValues(transmit, \.hwAlcEnabled) },
            _transmit!.observe(\.txInWaterfallEnabled, options: [.initial, .new]) { [weak self] (transmit, _) in
                self?.updateTransmitBoolValues(transmit, \.txInWaterfallEnabled )},
            
            // ----- Transmit Ints -----
            _transmit!.observe(\.maxPowerLevel, options: [.initial, .new]) { [weak self] (transmit, _) in
