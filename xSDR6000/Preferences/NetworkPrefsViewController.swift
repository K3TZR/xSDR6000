...
final class NetworkPrefsViewController: NSViewController {
    ...
    @IBAction func apply(_ sender: NSButton) {
        ...
        if _dhcpRadioButton.boolState {
            ...
            if _radio!.staticIp.isValidIP4() && _radio!.staticNetmask.isValidIP4() && _radio!.staticGateway.isValidIP4() {
                ...
                alert.beginSheetModal(for: NSApp.mainWindow!, completionHandler: { (_) in })
            }
        }
    }
    ...
    private func addObservations() {
        ...
        _observations = [
            // ----- Radio Strings -----
            _radio!.observe(\.ipAddress, options: [.initial, .new]) { [weak self] (radio, _) in
                self?.updateRadioStringValues(radio, \.ipAddress) },
            _radio!.observe(\.macAddress, options: [.initial, .new]) { [weak self] (radio, _) in
                self?.updateRadioStringValues(radio, \.macAddress) },
            _radio!.observe(\.netmask, options: [.initial, .new]) { [weak self] (radio, _) in
                self?.updateRadioStringValues(radio, \.netmask) },
            _radio!.observe(\.staticIp, options: [.initial, .new]) { [weak self] (radio, _) in
                self?.updateRadioStringValues(radio, \.staticIp) },
            _radio!.observe(\.staticNetmask, options: [.initial, .new]) { [weak self] (radio, _) in
                self?.updateRadioStringValues(radio, \.staticNetmask) },
            _radio!.observe(\.staticGateway, options: [.initial, .new]) { [weak self] (radio, _) in
                self?.updateRadioStringValues(radio, \.staticGateway) },
            
            // ----- Radio Bools -----
            _radio!.observe(\.enforcePrivateIpEnabled, options: [.initial, .new]) { [weak self] (radio, _) in
    ...
    private func updateRadioStringValues(_ radio: Radio, _ keypath: KeyPath<Radio, String>) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            case \.staticIp:
                if radio.staticIp.isEmpty && radio.staticNetmask.isEmpty && radio.staticGateway.isEmpty {
