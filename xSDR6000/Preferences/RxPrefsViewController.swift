...
final class RxPrefsViewController: NSViewController {
    ...
    private func addObservations() {
        ...
        _observations = [
            _radio!.observe(\.calFreq, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._calFreqTextField.integerValue = radio.calFreq }},
            
            _radio!.observe(\.freqErrorPpb, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._calOffsetTextField.integerValue = radio.freqErrorPpb }},
            
            _radio!.observe(\.snapTuneEnabled, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._snapTuneCheckbox.boolState = radio.snapTuneEnabled }},
            
            _radio!.observe(\.muteLocalAudio, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._muteLocalAudioCheckbox.boolState = radio.muteLocalAudio }},
            
            _radio!.observe(\.binauralRxEnabled, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
