...
final class FiltersPrefsViewController: NSViewController {
    ...
    private func addObservations() {
        _observations = [
            _radio!.observe(\.filterVoiceLevel, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._voiceSlider.integerValue = radio.filterVoiceLevel }},
            
            _radio!.observe(\.filterCwLevel, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._cwSlider.integerValue = radio.filterCwLevel }},
            
            _radio!.observe(\.filterDigitalLevel, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._digitalSlider.integerValue = radio.filterDigitalLevel }},
            
            _radio!.observe(\.filterVoiceAutoEnabled, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._voiceAutoCheckbox.boolState = radio.filterVoiceAutoEnabled }},
            
            _radio!.observe(\.filterCwAutoEnabled, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
                    self?._cwAutoCheckbox.boolState = radio.filterCwAutoEnabled }},
            
            _radio!.observe(\.filterDigitalAutoEnabled, options: [.initial, .new]) { [weak self] (radio, _) in
                DispatchQueue.main.async {
